...
/**
 * Micrometer Prometheus Grafana 整合观察指标数据启动器
 *
 * <pre>
 * === Prometheus 自动装配 ===
 *
 * 1. 引入依赖 micrometer-registry-prometheus
 * 2. Spring Boot Actuator 自动装配类 {@link PrometheusMetricsExportAutoConfiguration}
 ...
 *  2.1 当前工作目录：
 *      ./actuator-samples/micrometer-metrics-prometheus
 *  2.2 在工作目录执行如下命令：
 *      启动：
...
public class MicrometerMetricsPrometheusBootstrap {
    public static void main(String[] args) throws InterruptedException {
        ConfigurableApplicationContext ctx = SpringApplication.run(MicrometerMetricsPrometheusBootstrap.class, args);
