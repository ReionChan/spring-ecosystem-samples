...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<WebResponse<?>> handleValidateException(WebExchangeBindException e) {
        log.error("客户端参数异常: {}", e.getFieldError().getDefaultMessage());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(WebResponse.fail(e.getFieldError().getDefaultMessage()).build());
    }
